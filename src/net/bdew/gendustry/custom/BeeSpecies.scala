...
class BeeSpecies(cfg: ConfigSection, ident: String) extends IAlleleBeeSpecies {
  ...
  override def getProductChances = products.map(x => x._1 -> Float.box(x._2 / 100F)).asJava
  override def getSpecialtyChances = specialty.map(x => x._1 -> Float.box(x._2 / 100F)).asJava
